package solar

import (
	"image/color"
	"math"
	"math/rand/v2"

	rl "github.com/gen2brain/raylib-go/raylib"
)

type Planet struct {
	current  rl.Vector3
	previous rl.Vector3
	accel    rl.Vector3
	radius   float64
	mass     float64
	color    color.RGBA
}

func UpdatePositions(planets []Planet, delta_time float32) {
	for i := range planets {
		p := &planets[i]
		displacement := rl.Vector3Subtract(p.current, p.previous)
		p.previous = p.current
		p.current = rl.Vector3Add(
			rl.Vector3Add(p.current, displacement),
			rl.Vector3Scale(p.accel, delta_time*delta_time))
		p.accel = rl.Vector3{
			X: 0.0,
			Y: 0.0,
			Z: 0.0,
		}
	}
}

func ApplyGravity(planets []Planet) {
	gravity := rl.Vector3{
		X: 0.0,
		Y: -1000.0,
		Z: 0.0,
	}
	for i := range planets {
		p := &planets[i]
		p.accel = rl.Vector3Add(p.accel, gravity)
		displacement := rl.Vector3Subtract(p.current, p.previous)
		distance := math.Sqrt(float64(rl.Vector3DotProduct(displacement, displacement)))
		if distance > 0 {
			// Normalize the displacement vector
			n := rl.Vector3Scale(displacement, float32(1.0/distance))
			// Apply friction: accel += n * friction
			frictionForce := rl.Vector3Scale(n, float32(-200))
			p.accel = rl.Vector3Add(p.accel, frictionForce)
		}
	}
}

func ApplyGravity2(planets []Planet, gravity float64) {
	for i := range planets {
		for j := range planets {
			if i != j { // Avoid self-interaction
				// Calculate displacement vector between the two planets
				displacement := rl.Vector3Subtract(planets[j].current, planets[i].current)

				// Calculate the distance between the two planets
				distance := math.Sqrt(float64(rl.Vector3DotProduct(displacement, displacement)))

				if distance > 0 {
					// Calculate gravitational force magnitude: F = G * (m1 * m2) / r^2
					forceMagnitude := gravity * float64(planets[i].mass*planets[j].mass) / (distance * distance)

					// Normalize displacement to get the direction of the force
					direction := rl.Vector3Scale(displacement, float32(1.0/distance))

					// Calculate the force vector: F = direction * forceMagnitude
					force := rl.Vector3Scale(direction, float32(forceMagnitude))

					// Update the acceleration: a = F / m (F = ma, so a = F/m)
					accel := rl.Vector3Scale(force, float32(1.0/planets[i].mass))
					planets[i].accel = rl.Vector3Add(planets[i].accel, accel)
				}
			}
		}
	}
}

func ApplyConstraints(radius float64, verlet_objects []Planet) {
	for i := range verlet_objects {
		p := &verlet_objects[i]
		container_pos := rl.Vector3{
			X: 0.0,
			Y: 0.0,
			Z: 0.0,
		}
		displacement := rl.Vector3Subtract(p.current, container_pos)
		distance := math.Sqrt(float64(rl.Vector3DotProduct(displacement, displacement)))
		if distance > radius-p.radius {
			n := rl.Vector3Scale(displacement, float32(1.0/distance))
			p.current = rl.Vector3Add(container_pos, rl.Vector3Scale(n, float32(radius-p.radius)))
		}
	}
}

func RandomPointInSphere(radius float32) rl.Vector3 {
	for {
		// Generate random point in a cube [-1, 1] on each axis
		x := rand.Float32()*2 - 1
		y := rand.Float32()*2 - 1
		z := rand.Float32()*2 - 1

		// Check if the point is within the sphere
		if x*x+y*y+z*z <= 1 {
			return rl.Vector3{
				X: float32(x * radius),
				Y: float32(y * radius),
				Z: float32(z * radius),
			}
		}
	}
}

func Run() {
	rl.InitWindow(1280, 800, "Gravitational Pull Simulation")
	defer rl.CloseWindow()
	rl.SetTargetFPS(60)
	planets := make([]Planet, 2)
	sphere_radius := 15.0

	const G float64 = 6.67430e-11 // Gravitational constant
	time_step := 0.0015
	sub_steps := 1

	for i := range planets {
		random_pos := RandomPointInSphere(float32(sphere_radius))
		planets[i] = Planet{
			current:  random_pos,
			previous: random_pos,
			accel: rl.Vector3{
				X: 0.0,
				Y: 0.0,
				Z: 0.0,
			},
			radius: 1.0,
			mass:   10.0,
			color:  rl.Yellow,
		}
	}

	camera := rl.Camera3D{
		Position:   rl.Vector3{X: 25.0, Y: 25.0, Z: 25.0},
		Target:     rl.Vector3{X: 0.0, Y: 0.0, Z: 0.0},
		Up:         rl.Vector3{X: 0.0, Y: 1.0, Z: 0.0},
		Fovy:       45.0,
		Projection: rl.CameraPerspective,
	}
	for !rl.WindowShouldClose() {
		rl.BeginDrawing()
		rl.ClearBackground(rl.Black)
		rl.BeginMode3D(camera)

		for _, p := range planets {
			rl.DrawSphere(p.current, float32(p.radius), p.color)
		}
		rl.DrawSphereWires(rl.Vector3{
			X: 0.0,
			Y: 0.0,
			Z: 0.0,
		}, float32(sphere_radius), 16, 16, rl.DarkGray)

		sub_dt := float32(time_step) / float32(sub_steps)
		UpdatePositions(planets, sub_dt)
		// ApplyGravity(planets)
		ApplyGravity2(planets, G)
		ApplyConstraints(sphere_radius, planets)
		rl.EndMode3D()
		rl.EndDrawing()
	}
}
